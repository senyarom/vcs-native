...
HostInputState display_window_input() {
    ...
    if (const PfnXInputGetState get_state = xinput_get_state()) {
        ...
        if (get_state(0u, &pad) == 0u) {
            ...
            // Keep shoulder/face buttons as aliases. In vehicles, triggers
            // only drive: sending PSP R as well would suppress the throttle.
            if (!driving) {
                if (pad.gamepad.left_trigger > 64u) input.buttons |= kPspRTrigger;
                if (pad.gamepad.right_trigger > 64u) input.buttons |= kPspCircle;
            }
            // Vehicle accessors read these flags independently of the game's
            // ModernControlScheme setting.
