...
void apply_rendering_key(VcsConfiguration &config, const std::string &key,
                         const std::string &value, std::size_t line) {
    if (key == "texturefilter" || key == "mipmapping" || key == "mipmapfilter") {
        const auto v = lowercase_copy(trim_copy(value));
        if (key == "texturefilter") {
            if (v == "game") config.rendering.texture_filter = TextureFilter::Game;
            else if (v == "nearest") config.rendering.texture_filter = TextureFilter::Nearest;
            else if (v == "bilinear" || v == "linear") config.rendering.texture_filter = TextureFilter::Bilinear;
            else warning(config, line, "TextureFilter expects Game, Nearest or Bilinear");
        } else if (key == "mipmapping") {
            if (v == "game") config.rendering.mipmapping = MipmapMode::Game;
            else if (v == "off") config.rendering.mipmapping = MipmapMode::Off;
            else if (v == "on") config.rendering.mipmapping = MipmapMode::On;
            else warning(config, line, "Mipmapping expects Game, Off or On");
        } else {
            if (v == "game") config.rendering.mipmap_filter = MipmapFilter::Game;
            else if (v == "nearest") config.rendering.mipmap_filter = MipmapFilter::Nearest;
            else if (v == "linear") config.rendering.mipmap_filter = MipmapFilter::Linear;
            else warning(config, line, "MipmapFilter expects Game, Nearest or Linear");
        }
        return;
    }
    if (key == "backend" || key == "renderingbackend") {
        ...
        if (backend == "software" || backend == "cpu")
            ...
            warning(config, line, "Rendering.Backend expects Software, DirectX12 or Vulkan");
        return;
    }
...
void apply_timing_key(VcsConfiguration &config, const std::string &key,
    ...
    if (key == "framerate" || key == "fps" || key == "targetfps") {
        if (lowercase_copy(trim_copy(value)) == "uncapped" || trim_copy(value) == "0") {
            config.timing.frame_rate = 0u;
            return;
        }
        ...
        if (!parse_u32(value, 30u, 240u, frame_rate) ||
            ...
            warning(config, line, "Timing.FrameRate expects 30, 60, 120, 200, 240 or Uncapped");
            return;
        }
...
DisplaySurfaceDimensions resolve_display_surface_dimensions(
    ...
    case DisplayResolutionMode::Desktop:
#if defined(__APPLE__)
        if (auto mode = CGDisplayCopyDisplayMode(CGMainDisplayID())) {
            const DisplaySurfaceDimensions size{
        ...
        return {configuration.custom_width, configuration.custom_height};
#elif defined(_WIN32)
...
InternalResolutionDimensions resolve_internal_resolution(
    ...
    case InternalResolutionMode::Desktop:
#if defined(__APPLE__)
        {
            ...
            display.resolution_mode = DisplayResolutionMode::Desktop;
            display.custom_width = configuration.internal_width;
            display.custom_height = configuration.internal_height;
            const auto pixels = resolve_display_surface_dimensions(display);
            return fit_psp_aspect(pixels.width, pixels.height);
        }
#elif defined(_WIN32)
...
VcsConfiguration load_vcs_configuration(const std::filesystem::path &path) {
    ...
    while (std::getline(input, raw_line)) {
        ...
        if (section == "textures") {
            if (key == "enabled") {
                if (!parse_bool(value, config.texture_replacements))
                    warning(config, line_number, "Textures.Enabled expects true/false");
            } else if (key == "directory") config.texture_directory = value;
            else warning(config, line_number, "unknown [Textures] key '" + key + "'");
        } else if (section == "graphicsdistance") {
            float multiplier = 1.0f;
            if (!parse_float(value, kMinGraphicsDistance, kMaxGraphicsDistance, multiplier))
                warning(config, line_number, "GraphicsDistance expects a multiplier between 1 and 10");
            else if (key == "world") config.draw_distance.world = multiplier;
            else if (key == "lod") config.draw_distance.lod = multiplier;
            else warning(config, line_number, "unknown GraphicsDistance key");
...
void initialize_vcs_configuration(const std::filesystem::path &executable_directory) {
    ...
    if (std::getenv("PSPRECOMP_GE_BACKEND") == nullptr) {
        set_environment_value("PSPRECOMP_GE_BACKEND",
                              config.rendering.backend == RenderingBackend::DirectX12
                                  ? "directx12" : config.rendering.backend == RenderingBackend::Vulkan
                                  ? "vulkan" : "software");
    }
...
void publish_graphics_configuration(const VcsConfiguration &config) {
    auto &live = global_configuration();
    live.rendering = config.rendering;
    live.display = config.display;
    live.timing.frame_rate = config.timing.frame_rate;
    live.draw_distance = config.draw_distance;
    live.texture_replacements = config.texture_replacements;
}
