...
int main(int argc, char **argv) {
    ...
    try {
        ...
        vcs::runtime_log_line("bootstrap image=built-in ULUS10160 1.03");
        ...
        // The native world cache uses the upper 32 MiB; PSP heap/stacks still end at 0x0A000000.
        psprecomp::Runtime runtime((vcs::world_lod_cache_directory().empty() ? 32u : 64u) * 1024u * 1024u);
        vcs::runtime_log_line("world LOD cache=" + vcs::world_lod_cache_directory().string() +
            " RAM_MiB=" + std::to_string(runtime.memory().size() / (1024u * 1024u)));
        runtime.set_game_root(root);
        vcs::initialize_game_memory(runtime.memory());
        const auto &boot = vcs::game_boot_image();
        const std::uint32_t user_arena_start = boot.arena_start;
        ...
        std::cout << "VCSNative PSP bootstrap\n"
                  ...
                  << "Game image: built-in ULUS10160 1.03 (relocated at import)\n"
                  << "Source SHA-256: " << boot.source_sha256 << "\n"
                  << "Entry:     " << psprecomp::hex32(boot.entry) << "\n"
        ...
        if (configuration.rendering.smaa && gpu_start.active == vcs::GeGpuBackendKind::Vulkan) {
            std::cout << "SMAA requested but not implemented on Vulkan GE\n";
        } else if (configuration.rendering.smaa && gpu_start.active == vcs::GeGpuBackendKind::DirectX12) {
        ...
        if (gpu_start.active == vcs::GeGpuBackendKind::Vulkan) {
            std::cout << "Vulkan presentation:  GPU scene rendering, final RGBA frame copied to SDL\n";
            if (configuration.volumetric_clouds.enabled)
                std::cout << "Volumetric clouds:    requested but not implemented on Vulkan GE\n";
        }
        ...
        runtime.cpu().set_gpr(28, boot.gp);
        ...
        runtime.run(boot.entry, max_dispatches);
        ...
        if (gpu_final.requested == vcs::GeGpuBackendKind::Vulkan) {
            std::cout << "GE Vulkan: active=" << vcs::ge_gpu_backend_name(gpu_final.active)
                      << " frames=" << gpu_final.game_frames
                      << " draws=" << gpu_final.game_draw_calls
                      << " hw_draws=" << gpu_final.hw_transform_draw_calls
                      << " feedback=" << gpu_final.vram_feedback_refreshes
                      << " missing_textures=" << gpu_final.missing_texture_draw_calls
                      << " unsupported_blend=" << gpu_final.unsupported_blend_game_draw_calls
                      << " target=" << gpu_final.offscreen_width << 'x' << gpu_final.offscreen_height
                      << " readback_bytes_per_frame=" << gpu_final.game_frame_readback_bytes
                      << "\n";
        }
        ...
        vcs::reset_texture_replacements();
