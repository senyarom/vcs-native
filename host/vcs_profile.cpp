...
bool parse_atrac_header(std::span<const std::uint8_t> bytes, ParsedAtracHeader &header) {
    ...
    for (std::size_t offset = 12u; offset + 8u <= bytes.size();) {
        ...
        if (std::memcmp(bytes.data() + offset, "fmt ", 4u) == 0 && chunk_size >= 16u) {
            ...
            if (header.format_tag == 0xFFFEu) {
                // WAVEFORMATEXTENSIBLE identifies the codec with SubFormat.
                // Block alignment is a bitrate-dependent size, not a codec ID:
                // NEWS_* uses ATRAC3+ with 280-byte frames, CITY/WATER use 376.
                constexpr std::array<std::uint8_t, 16> atrac3plus_guid{
                    0xBF, 0xAA, 0x23, 0xE9, 0x58, 0xCB, 0x71, 0x44,
                    0xA1, 0x19, 0xFF, 0xFA, 0x01, 0xE4, 0xCE, 0x62};
                if (chunk_size < 40u || read_le16(bytes, payload + 16u) < 22u ||
                    read_le16(bytes, payload + 16u) > chunk_size - 18u ||
                    std::memcmp(bytes.data() + payload + 24u, atrac3plus_guid.data(),
                                atrac3plus_guid.size()) != 0) return false;
                header.atrac3plus = true;
            } else if (header.format_tag == 0x0270u) {
                header.atrac3plus = false;
            } else {
                return false;
            }
            have_fmt = true;
...
std::filesystem::path identify_atrac_source(std::span<const std::uint8_t> header, const ParsedAtracHeader &parsed) {
    // NEWS_1 and NEWS_2 have equal file sizes and the same first 3290 bytes
    // (including silent encoded frames). Compare all supplied bytes, and do
    // not pick an arbitrary unordered-map entry if the prefix is ambiguous.
    const std::size_t compare_size = std::min<std::size_t>(header.size(), parsed.file_size);
...
GraphicsRealtimeClock uncapped_clock;
GraphicsFramePacer display_pacer;
std::chrono::steady_clock::duration scheduler_idle_time{};
bool frame_time_diag_enabled();
std::uint64_t graphics_clock_revision{};
std::uint32_t graphics_clock_frame_rate{};
std::uint32_t configured_game_frame_rate() noexcept {
    return patches::mission_frame_rate(vcs_configuration().timing.frame_rate);
}
bool uncapped_graphics() noexcept { return configured_game_frame_rate() == 0; }
bool realtime_graphics_clock() noexcept {
    static const bool capped=[] {
        const auto *value=std::getenv("PSPRECOMP_REALTIME_CAPPED");
        if (value) return std::strcmp(value,"1")==0;
    ...
    return uncapped_graphics() || (capped && vcs_configuration().initialized);
}
void update_uncapped_clock() { if (realtime_graphics_clock()) uncapped_clock.update(virtual_time_us); }
void reset_graphics_clock() {
    uncapped_clock.reset(virtual_time_us);
    display_pacer.reset();
    graphics_clock_frame_rate = configured_game_frame_rate();
    ++graphics_clock_revision;
}
...
constexpr bool ge_command_affects_lighting(std::uint32_t command) noexcept {
...
std::filesystem::path translate_io_path(const psprecomp::Runtime &runtime, const std::string &guest_path) {
    // Validate/normalize first, including traversal rejection. The runtime's
    // default device mapping is the disc root; saves have a separate host mount.
    const auto disc_path = runtime.translate_path(guest_path);
    const auto colon = guest_path.find(':');
    const auto equal_ascii = [](std::string a, const char *b) {
        for (auto &c : a) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        return a == b;
    };
    const auto device = guest_path.substr(0, colon);
    if (colon == std::string::npos || (!equal_ascii(device, "ms0") && !equal_ascii(device, "fatms0")))
        return disc_path;
    const auto relative = disc_path.lexically_relative(runtime.game_root());
    auto part = relative.begin();
    if (part == relative.end() || !equal_ascii(part->string(), "psp")) return disc_path;
    ++part;
    if (part == relative.end() || !equal_ascii(part->string(), "savedata")) return disc_path;
    auto native = savedata_root(runtime);
    for (++part; part != relative.end(); ++part) native /= *part;
    // VCS's startup uses MS0:PSP/SAVEDATA with sceIoDopen/Getstat to choose
    // the newest slot; utility LOAD/SAVE must see exactly the same directory.
    return native;
}
...
std::uint32_t save_savedata_file(psprecomp::Runtime &runtime, std::uint32_t parameter_address,
    ...
    // Overwriting existing files does not refresh their parent directory on
    // host filesystems. VCS chooses the last save by the slot directory's mtime.
    std::error_code error;
    std::filesystem::last_write_time(path.parent_path(), std::filesystem::file_time_type::clock::now(), error);
    if (error) return raw_mode ? 0x80110329u : 0x80110385u;
    return 0u;
}
...
std::uint64_t audio_queue_buffer(AudioChannelState &channel, std::uint32_t frames) {
    ...
    if (!channel.queue_active || (start < virtual_time_us &&
        virtual_time_us - start > vcs::kAudioSchedulingSlackUs)) {
        // A large gap starts a new stream. Small scheduler delays retain the
        // DAC sample timeline: the host mixer still holds this region, and
        // the zero-duration blocking return lets the audio thread catch up.
        // Re-anchoring on ANY lateness inserted silence on nearly every radio
        // buffer when a rendered frame delayed the guest thread by a few ms.
...
void vcs_load_codec_modules(psprecomp::Runtime &runtime, psprecomp::AllegrexContext &ctx) {
    ...
    for (std::size_t index = 0u; index < 64u; ++index) {
        ...
        const auto native = translate_io_path(runtime, guest_path);
...
void diagnose_scheduler_stall(const psprecomp::AllegrexContext &ctx);
...
bool activate_next_thread(psprecomp::AllegrexContext &ctx, const char *reason) {
    ...
    if (thread_table.continuations.empty()) {
        ...
        if (earliest != UINT64_MAX) {
            ...
            if (realtime_graphics_clock()) {
                // Sleep only when no PSP thread can run. A capped display
                // wait uses the same deadline queue as loader/audio workers.
                const bool measured = frame_time_diag_enabled();
                const auto before = measured ? std::chrono::steady_clock::now()
                                             : std::chrono::steady_clock::time_point{};
                const auto deadline = uncapped_clock.deadline(earliest);
#if defined(__linux__)
                // Match the old limiter's low-latency tail, but only while
                // every PSP thread is blocked. Frequent full sleeps let
                // autonomous CPU power management undershoot the next frame's
                // demand, as well as adding scheduler wakeup jitter.
                const auto margin = std::chrono::microseconds(
                    configured_game_frame_rate() >= 60u ? 1500 : 0);
                std::this_thread::sleep_until(deadline - margin);
                while (std::chrono::steady_clock::now() < deadline) std::this_thread::yield();
#else
                std::this_thread::sleep_until(deadline);
#endif
                if (measured) scheduler_idle_time += std::chrono::steady_clock::now() - before;
                update_uncapped_clock();
            } else virtual_time_us = std::max(virtual_time_us, earliest);
            promote_expired_delays();
        }
    }
...
// Only register writes with no control-flow, primitive, or streaming-matrix
// side effects. Keep action commands on the ordinary dispatcher below.
constexpr auto kGeRegisterFlags=[] {
    std::array<std::uint8_t,256> flags{};
    for(unsigned c=0x15;c<256;++c) {
        if(c>=0x2a && c<=0x41) continue;
        flags[c]=8u | (ge_command_affects_gpu_draw_descriptor(c)?1u:0u) |
            (ge_command_affects_lighting(c)?2u:0u) |
            (((c>=0x42 && c<=0x47) || c==0x4c || c==0x4d)?4u:0u);
    }
    return flags;
}();
...
struct FrameTimeStats {
    ...
    std::chrono::steady_clock::duration pacing_time{};
...
void dump_gpu_internal_frame_if_requested(std::uint64_t vblank) {
    static std::uint64_t dumped = 0u;
    // Consecutive frames expose alternating-buffer bugs that a single still
    // cannot show. The default remains one capture at the requested VBlank.
    static const std::uint64_t count = std::clamp<std::uint64_t>(
        parse_environment_u64("PSPRECOMP_GE_GPU_DUMP_COUNT", 1u), 1u, 120u);
    static const std::uint64_t interval = std::max<std::uint64_t>(1u,
        parse_environment_u64("PSPRECOMP_GE_GPU_DUMP_INTERVAL", 1u));
    const std::uint64_t requested = gpu_dump_vblank();
    if (dumped >= count || requested == 0u || vblank < requested ||
        (vblank - requested) / interval < dumped) return;
    ...
    if (count > 1u) {
        output_path = output_path.parent_path() /
            (output_path.stem().string() + "_" + std::to_string(report.game_frame_vblank) +
...
void report_realtime_speed_if_requested() {
    ...
    if (!uncapped_graphics() && configured_game_frame_rate() <= 60 &&
        (sample.guest_us_per_vblank < 15'000.0 || sample.guest_us_per_vblank > 18'500.0)) {
...
// A deterministic headless integration-test driver for the same Apply path as
// F10. The file contains "vblank quoted-INI-path" lines. No filesystem polling
// or extra work occurs during normal play.
bool apply_graphics_test_sequence(psprecomp::Runtime &rt, std::uint64_t vblank) {
    static auto sequence = [] {
        std::vector<std::pair<std::uint64_t, std::string>> values;
        const char *path = std::getenv("PSPRECOMP_GRAPHICS_TEST_SEQUENCE");
        if (!path || !*path) return values;
        std::ifstream file(path);
        if (!file) throw std::runtime_error("Cannot open graphics test sequence");
        std::uint64_t frame; std::string ini;
        while (file >> frame >> std::quoted(ini)) values.emplace_back(frame,ini);
        if (!file.eof()) throw std::runtime_error("Malformed graphics test sequence");
        return values;
    }();
    static std::size_t index = 0;
    if (index >= sequence.size() || sequence[index].first > vblank) return false;
    const auto next = load_vcs_configuration(sequence[index++].second);
    if (!next.loaded_from_file) throw std::runtime_error("Cannot load graphics test settings");
    std::string error;
    if (!apply_live_graphics_settings(rt, graphics_settings_from(next), error))
        throw std::runtime_error("Graphics test Apply failed: " + error);
    std::cerr << "[graphics-test] applied at vblank=" << vblank << "\n";
    return true;
}
...
void limit_frame_rate() {
    // With a wall clock the vblank deadline belongs to the waiting PSP
    // thread. Sleeping here would block every emulated thread, then release
    // the high-priority display thread immediately and starve the loader.
    if (realtime_graphics_clock()) { update_uncapped_clock(); return; }
    if (!frame_limit_enabled()) return;

    static bool anchored = false;
    static std::uint64_t revision = 0;
    if (revision != graphics_clock_revision) { anchored = false; revision = graphics_clock_revision; }
...
bool execute_ge_list(psprecomp::Runtime &runtime, GeListRecord &list,
    ...
    const bool trace_control = std::getenv("PSPRECOMP_GE_CONTROL_TRACE") != nullptr;
    static const bool register_stream_enabled=[] {
        const auto *value=std::getenv("PSPRECOMP_GE_REGISTER_STREAM");
        return !value || std::strcmp(value,"0")!=0;
    }();
    std::array<std::array<std::uint32_t,4>,32> control_history{};
    ...
    while (list.executed_commands < kMaximumCommandsPerRun) {
        ...
        if ((list.pc & 3u) != 0u || !fetch_ge_command(list.pc, op)) {
            ...
            if (trace_control) {
                for (std::size_t i=0;i<std::min(control_index,control_history.size());++i) {
                    const auto first=control_index>control_history.size()?control_index:0;
                    const auto &item=control_history[(first+i)&31];
                    std::cerr<<"[ge-control] pc="<<psprecomp::hex32(item[0])<<" op="<<psprecomp::hex32(item[1])
                             <<" base="<<psprecomp::hex32(item[2])<<" offset="<<psprecomp::hex32(item[3])<<'\n';
                }
                if (const char *path=std::getenv("PSPRECOMP_GE_FAILURE_DUMP")) {
                    std::ofstream dump(path,std::ios::binary);
                    const auto &bytes=runtime.memory().bytes();
        ...
        if (trace_control && command>=8 && command<=0x14)
            control_history[control_index++ & 31]={op_pc,op,ge_state.commands[0x10],ge_state.offset_address};
        static const bool matrix_stream_enabled=[] {
            const auto *value=std::getenv("PSPRECOMP_GE_MATRIX_STREAM");
            return !value || std::strcmp(value,"0")!=0;
        }();
        // Synchronous matrix DATA runs cannot invoke callbacks or branch. Decode
        // the run without repeating the general register/switch dispatch per word.
        // The async worker keeps the original per-word acquire/stall behavior.
        if(matrix_stream_enabled && !async_stall && (command==0x2b || command==0x3b ||
            command==0x3d || command==0x3f || command==0x41)) {
            const auto offset=std::size_t(op_pc-ge_fetch_guest_base);
            auto words=std::min<std::size_t>((ge_fetch_bytes-offset)/4,
                kMaximumCommandsPerRun-list.executed_commands);
            words=std::min<std::size_t>(words,(0x10000000u-op_pc)/4);
            if(list.stall>op_pc && !(list.stall&3u))
                words=std::min<std::size_t>(words,(list.stall-op_pc)/4);
            const auto consumed=consume_ge_matrix_stream(ge_state.transform,command,
                ge_fetch_pointer+offset,words);
            if(consumed) {
                ge_state.commands[command]=read_le32_unaligned(ge_fetch_pointer+offset+(consumed-1)*4);
                if(command==0x3d || command==0x3f) ge_camera_state_revision+=consumed;
                if(ge_histogram) list.histogram[command]+=consumed;
                if(count_ge_commands) ge_commands_this_vblank+=consumed;
                list.executed_commands+=consumed;
                list.pc=(op_pc+std::uint32_t(consumed)*4)&0x0fffffffu;
                continue;
            }
        }
        if(register_stream_enabled && !scalar_registers && !async_stall && kGeRegisterFlags[command]) {
            const auto offset=std::size_t(op_pc-ge_fetch_guest_base);
            auto words=std::min<std::size_t>((ge_fetch_bytes-offset)/4,
                kMaximumCommandsPerRun-list.executed_commands);
            words=std::min<std::size_t>(words,(0x10000000u-op_pc)/4);
            if(list.stall>op_pc && !(list.stall&3u)) words=std::min<std::size_t>(words,(list.stall-op_pc)/4);
            std::size_t consumed=0;
            for(;consumed<words;++consumed) {
                const auto value=read_le32_unaligned(ge_fetch_pointer+offset+4*consumed);
                const auto cmd=value>>24u, flags=std::uint32_t(kGeRegisterFlags[cmd]);
                if(!flags) break;
                if(value!=ge_state.commands[cmd]) {
                    ge_state.commands[cmd]=value;
                    ge_draw_state_revision+=flags&1u;
                    ge_lighting_state_revision+=(flags>>1)&1u;
                    ge_camera_state_revision+=(flags>>2)&1u;
                }
                if(ge_histogram) ++list.histogram[cmd];
            }
            if(count_ge_commands) ge_commands_this_vblank+=consumed;
            list.executed_commands+=consumed;
            list.pc=(op_pc+std::uint32_t(consumed)*4)&0x0fffffffu;
        ...
        if (command >= 0x3Cu && command <= 0x3Fu)
            ...
            ++ge_camera_state_revision;
        // Only matrix/morph commands 0x2A..0x41 are consumed here. Avoid a
...
namespace {
// A guest wait can leave audio running while the game never reaches another
// display import. Diagnose from scheduler activity, independently of vblank.
void diagnose_scheduler_stall(const psprecomp::AllegrexContext &ctx) {
    static const bool enabled = std::getenv("PSPRECOMP_SCHED_STALL_DIAG") != nullptr;
    if (!enabled) return;
    static std::uint64_t last_vblank = 0u;
    static auto last_progress = std::chrono::steady_clock::now();
    const auto now = std::chrono::steady_clock::now();
    if (last_vblank != display_vblank_index) {
        last_vblank = display_vblank_index;
        last_progress = now;
    }
    if (now - last_progress < std::chrono::seconds(3)) return;
    last_progress = now;
    dump_event_stall_state(EventFlagRecord{}, -1, ctx);
    for (const auto &[uid, flag] : event_flag_table.flags) {
        std::cerr << "[scheduler-stall-event] uid=" << uid << " name=" << flag.name
                  << " pattern=" << flag.current_pattern << " waiters=" << flag.waiters.size() << '\n';
        for (const auto &waiter : flag.waiters)
            std::cerr << "[scheduler-stall-waiter] event=" << uid << " thread=" << waiter.uid
                      << " requested=" << waiter.requested << " mode=" << waiter.mode
                      << " pc=" << psprecomp::hex32(waiter.context.pc) << '\n';
    }
    for (const auto &[uid, semaphore] : semaphore_table.semaphores) {
        std::cerr << "[scheduler-stall-semaphore] uid=" << uid << " name=" << semaphore.name
                  << " count=" << semaphore.count << " waiters=" << semaphore.waiters.size() << '\n';
        for (const auto &waiter : semaphore.waiters)
            std::cerr << "[scheduler-stall-waiter] semaphore=" << uid << " thread=" << waiter.uid
                      << " pc=" << psprecomp::hex32(waiter.context.pc) << '\n';
    }
    for (const auto &[uid, callbacks] : pending_guest_callbacks)
        for (const auto &callback : callbacks)
            std::cerr << "[scheduler-stall-callback] thread=" << uid
                      << " function=" << psprecomp::hex32(callback.function) << '\n';
    for (std::size_t id = 0; id < atrac_contexts.size(); ++id) {
        const auto &stream = atrac_contexts[id];
        if (!stream.allocated) continue;
        std::cerr << "[scheduler-stall-atrac] id=" << id << " source=" << stream.source_path
                  << " sample=" << stream.sample_position << " total=" << stream.header.total_samples
                  << " eof=" << stream.decoder_eof << " error=" << stream.internal_error << '\n';
    }
}
}
...
void install_profile(psprecomp::Runtime &runtime, std::uint32_t user_arena_start) {
    ...
    install_world_lod(runtime);
    // Diagnostic camera override at the handoff to the RenderWare frame. The
    // frame-end CCam fields are otherwise overwritten by gameplay camera logic.
    if (std::getenv("PSPRECOMP_TEST_WORLD_CAMERA_ONLY") != nullptr) {
        runtime.register_function(0x08A1A5D4u,
            [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
                const char *pose = std::getenv("PSPRECOMP_TEST_WORLD_POSE");
                float x, y, z;
                if (ctx.gpr[31] == 0x08A23EB4u && display_vblank_index >= 2800u &&
                    pose && std::sscanf(pose, "%f,%f,%f", &x, &y, &z) == 3 &&
                    std::isfinite(x) && std::isfinite(y) && std::isfinite(z)) {
                    {
                        const auto b=detail::test_camera_basis(std::getenv("PSPRECOMP_TEST_WORLD_DIRECTION"));
                        const float matrix[16] = {b.right[0],b.right[1],b.right[2],0,
                            b.forward[0],b.forward[1],b.forward[2],0,b.up[0],b.up[1],b.up[2],0,x,y,z,1};
                        for (unsigned i = 0; i < 16; ++i)
                            rt.memory().store32(ctx.gpr[16] + 4u*i,
                                std::bit_cast<std::uint32_t>(matrix[i]));
                    }
                }
                // Original leaf: return frame + 16 in v0, preserving all else.
                ctx.gpr[2] = ctx.gpr[4] + 16u;
                ctx.pc = ctx.gpr[31];
            }, "vcs_test_camera_frame");
    }
    fps_game_frame_reset();
    reset_texture_replacements();
    runtime.register_function(0x08A070DCu,
        [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
            fps_game_frame_completed();
            patches::advance_weapon_timing(rt,ctx.gpr[28]);
            update_world_streaming(rt.memory(), vcs_configuration().draw_distance.lod);
            // Read-only pose evidence for matching interactive slow views to
            // subsequent fixed-camera benchmarks. No writes to game/save state.
            static const bool capture_pose=std::getenv("PSPRECOMP_TEST_CAPTURE_POSE")!=nullptr;
            static unsigned pose_frames=0;
            if(capture_pose && ++pose_frames%120==0) {
                const auto read_float=[&](std::uint32_t p) { return std::bit_cast<float>(rt.memory().load32(p)); };
                constexpr std::uint32_t camera=0x08BC7E30u,position=camera+0x9B0u;
                std::fprintf(stderr,"[camera-pose] vblank=%llu position=%.5f,%.5f,%.5f direction=%.5f,%.5f,%.5f\n",
                    (unsigned long long)display_vblank_index,
                    read_float(position),read_float(position+4),read_float(position+8),
                    read_float(camera+0x80),read_float(camera+0x84),read_float(camera+0x88));
            }
            // Opt-in fixed pose for repeatable world-streaming captures (ULUS10160 1.03).
            if (const char *pose = std::getenv("PSPRECOMP_TEST_WORLD_POSE");
                pose != nullptr && display_vblank_index >= 2800u) {
                float x, y, z;
                if (std::sscanf(pose, "%f,%f,%f", &x, &y, &z) == 3 &&
                    std::isfinite(x) && std::isfinite(y) && std::isfinite(z)) {
                    const auto player = rt.memory().load32(0x08BDE4B0u +
                        352u * rt.memory().load8(ctx.gpr[28] + 7800u));
                    if (player >= 0x08800000u && rt.memory().contains(player, 0x900u)) {
                        auto put = [&](std::uint32_t address, float value) {
                            rt.memory().store32(address, std::bit_cast<std::uint32_t>(value));
                        };
                        if(std::getenv("PSPRECOMP_TEST_WORLD_CAMERA_ONLY")==nullptr) {
                            put(player + 0x30u, x); put(player + 0x34u, y); put(player + 0x38u, z - 1.5f);
                            rt.memory().store8(player + 0x550u, 1u);
                        }
                        constexpr auto cam = 0x08BC7E30u;
                        put(cam + 0x798u, 4.0f);
                        put(cam + 0x90u, x); put(cam + 0x94u, y); put(cam + 0x98u, z);
                        auto basis=detail::test_camera_basis(std::getenv("PSPRECOMP_TEST_WORLD_DIRECTION"));
                        // Diagnostic orbit covers eight repeatable base views,
                        // with no controller input and no saved camera changes.
                        if(std::getenv("PSPRECOMP_TEST_WORLD_ORBIT") && display_vblank_index>=3200u) {
                            const auto sector=std::min<std::uint64_t>(7,(display_vblank_index-3200)/600);
                            const float angle=float(sector)*0.7853981633974483f;
                            basis.forward={std::cos(angle),std::sin(angle),0};basis.up={0,0,1};
                        }
                        for(unsigned i=0;i<3;++i) {
                            put(cam+0x80u+4*i,basis.forward[i]);put(cam+0xD0u+4*i,basis.up[i]);
                        }
                        if(const char *hour=std::getenv("PSPRECOMP_TEST_WORLD_HOUR")) {
                            char *end=nullptr;const auto value=std::strtoul(hour,&end,10);
                            if(end!=hour && *end=='\0' && value<24)rt.memory().store8(ctx.gpr[28]+7648u,value);
                        }
                    }
                }
            }

            // Preserve the exact resume block after the game's frame limiter
            // (ULUS10160 1.03). Both stock 30 and unlocked 60 pass here once per
            // game frame; the intervening VBlank waits do not count as frames.
            ctx.gpr[6] = ctx.gpr[29] + 16u;
            ctx.gpr[7] = ctx.gpr[29] + 20u;
            ctx.gpr[5] = ctx.gpr[3];
            ctx.gpr[31] = 0x08A070F0u;
            ctx.gpr[4] = ctx.gpr[2];
            ctx.pc = 0x08B7343Cu;
        }, "vcs_game_frame_completed");
    patches::install_weapon_timing(runtime);
    patches::install_swimming_timing(runtime);
    patches::install_mission_fps_patches(runtime, [] { return vcs_configuration().timing.frame_rate; });
    ...
    runtime.register_hle("ThreadManForUser", 0x349D6D6Cu,
        [](psprecomp::Runtime &, psprecomp::AllegrexContext &ctx) {
            ...
            if (realtime_graphics_clock()) update_uncapped_clock();
            else virtual_time_us += 25u;
            promote_expired_delays();
    ...
    auto wait_vblank = [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
        ...
        if (!ge_async_wait_idle(rt)) return;
        patches::update_mission_fps_patches(rt, ctx.gpr[28]);
        if (graphics_clock_frame_rate != configured_game_frame_rate()) reset_graphics_clock();
        ...
        if (frame_time_diag_enabled()) {
            ...
            if (frame_time_stats.started) {
                ...
                frame_line << "[frame-time] vblank=" << display_vblank_index
                           ...
                           << " pacing_us=" << std::chrono::duration_cast<std::chrono::microseconds>(frame_time_stats.pacing_time).count()
                           << " scheduler_idle_us=" << std::chrono::duration_cast<std::chrono::microseconds>(scheduler_idle_time).count()
                           << " io_us=" << io_us
            ...
            frame_time_stats.pacing_time = std::chrono::steady_clock::duration{};
            scheduler_idle_time = {};
        ...
        const auto present_entry = frame_time_diag_enabled()
            ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
        fps_overlay_render_frame(display_state.frame_buffer);
        ...
        static bool logged_software_fallback = false;
        if (!presented_gpu_frame && gpu_has_presented && !logged_software_fallback) {
            const auto report = ge_gpu_backend_report();
            std::ostringstream message;
            message << "[presentation] GPU -> PSP framebuffer vblank=" << display_vblank_index
                    << " gpu_active=" << ge_gpu_backend_active()
                    << " framebuffer=" << psprecomp::hex32(display_state.frame_buffer)
                    << " last_gpu_target=" << psprecomp::hex32(report.presented_framebuffer_target)
                    << " last_gpu_vblank=" << report.game_frame_vblank
                    << " resolution=" << report.offscreen_width << 'x' << report.offscreen_height
                    << " status=" << report.message;
            runtime_log_line(message.str());
            logged_software_fallback = true;
        } else if (presented_gpu_frame && logged_software_fallback) {
            runtime_log_line("[presentation] GPU output restored at vblank=" + std::to_string(display_vblank_index));
            logged_software_fallback = false;
        }
        if (!presented_gpu_frame) {
            ...
            if (ge_gpu_backend_presents_directly()) ++swapchain_presents;
        }
        ...
        const bool test_settings_changed = apply_graphics_test_sequence(rt, display_vblank_index);
        const bool panel_opened = display_window_graphics_panel([&](const GraphicsSettings &settings, std::string &error) {
            return apply_live_graphics_settings(rt, settings, error);
        });
        if (panel_opened || test_settings_changed) {
            patches::update_mission_fps_patches(rt, ctx.gpr[28]);
            reset_graphics_clock();
            fps_game_frame_reset();
            realtime_speed_stats = {};
        }
        if(frame_time_diag_enabled()) {
            const auto pacing_start=std::chrono::steady_clock::now();
            limit_frame_rate();
            frame_time_stats.pacing_time+=std::chrono::steady_clock::now()-pacing_start;
        } else limit_frame_rate();
        ...
        const std::uint64_t period = virtual_vblank_period_us();
        std::uint32_t delay = static_cast<std::uint32_t>(period - (virtual_time_us % period));
        if (realtime_graphics_clock()) {
            delay = 1u;
            if (!uncapped_graphics() && frame_limit_enabled()) {
                const auto now = GraphicsRealtimeClock::Clock::now();
                uncapped_clock.update(virtual_time_us, now);
                delay = display_pacer.wait_microseconds(now, period);
            }
        }
        const auto current = thread_table.threads.find(thread_table.current_uid);
        if (current == thread_table.threads.end()) {
    ...
    runtime.register_hle("sceUtility", 0x50C4CD57u,
        [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
            ...
            if (std::getenv("PSPRECOMP_TRACE") != nullptr || std::getenv("PSPRECOMP_SAVEDATA_DIAG") != nullptr) {
    ...
    runtime.register_hle("IoFileMgrForUser", 0xB29DDF9Cu,
        [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
            try {
                const auto guest = rt.memory().read_c_string(ctx.gpr[4]);
                const auto native = translate_io_path(rt, guest);
                if (std::getenv("PSPRECOMP_SAVEDATA_DIAG") != nullptr)
                    std::cerr << "[savedata-io] dopen " << guest << " -> " << native.string()
                              << " exists=" << std::filesystem::is_directory(native) << '\n';
    ...
    // ScePspDateTime stamps and six private words. Startup uses st_mtime to
    // select the latest slot before invoking the savedata utility.
    runtime.register_hle("IoFileMgrForUser", 0xACE946E8u,
        [](psprecomp::Runtime &rt, psprecomp::AllegrexContext &ctx) {
            ...
            std::error_code error;
            const auto native = translate_io_path(rt, path);
            ...
            auto timestamp_path = native;
            if (directory && native.parent_path().lexically_normal() == savedata_root(rt).lexically_normal() &&
                std::filesystem::is_regular_file(native / "DATA.BIN", error)) {
                // Older native builds did not update the directory when a slot
                // was overwritten. Use the actual progress file for these saves
                // too, without rewriting/migrating the player's existing data.
                timestamp_path /= "DATA.BIN";
            }
            const auto written = std::filesystem::last_write_time(timestamp_path, error);
            const auto system_time = std::filesystem::file_time_type::clock::to_sys(written);
            const std::time_t seconds = std::chrono::system_clock::to_time_t(
                std::chrono::time_point_cast<std::chrono::system_clock::duration>(system_time));
            std::tm parts{};
#if defined(_WIN32)
...
void vcs_starvation_tick(psprecomp::Runtime &, psprecomp::AllegrexContext &ctx) {
    if (realtime_graphics_clock()) update_uncapped_clock();
    else virtual_time_us += starvation_tick_microseconds;
    promote_expired_delays();
...
bool run_profile_self_tests(std::string &error) {
    ...
    try {
        ...
        // Compare batched register writes against the complete scalar dispatcher
        // across a fetch-window boundary, stalls, a matrix stream and CALL/RET.
        {
            const auto saved=ge_state;
            psprecomp::Runtime rt;
            constexpr std::uint32_t pc=0x08050000;
            std::vector<std::uint32_t> ops;
            for(unsigned round=0;round<100;++round) {
                for(unsigned cmd=0x15;cmd<256;++cmd) {
                    if(cmd>=0x2a && cmd<=0x41) continue;
                    const auto op=(cmd<<24)|((round*7919+cmd*17)&0xffffff);
                    ops.push_back(op);ops.push_back(op); // identical writes do not change revisions
                }
                ops.push_back(0x3e00000f);ops.push_back(ge_float24_command(0x3f,.25f));
                ops.push_back(ge_float24_command(0x3f,.5f));
            }
            ops.push_back((kGeCommandBase<<24)|0x00080000);
            const auto call_index=ops.size();ops.push_back(0);
            ops.push_back(kGeCommandFinish<<24);ops.push_back(kGeCommandEnd<<24);
            const auto subroutine=pc+4*ops.size();
            ops[call_index]=(kGeCommandCall<<24)|(subroutine&0xffffff);
            ops.push_back(0xa0001234);ops.push_back(0x17000001);ops.push_back(kGeCommandReturn<<24);
            for(unsigned i=0;i<ops.size();++i)rt.memory().store32(pc+4*i,ops[i]);
            GeState reference{};GeListRecord reference_list{};
            std::array<std::uint64_t,3> reference_revisions{};
            for(bool scalar:{true,false}) {
                ge_state=GeState{};reset_ge_transform_state(ge_state.transform);
                ge_draw_state_revision=ge_lighting_state_revision=ge_camera_state_revision=1;
                GeListRecord list{};list.pc=pc;list.stack_capacity=16;
                std::vector<GuestCallbackInvocation> callbacks;
                for(unsigned cut:{23u,5000u,16384u,0u}) {
                    list.stall=cut?pc+4*cut:0;
                    require(execute_ge_list(rt,list,callbacks,nullptr,scalar),"register stream dispatcher failed");
                    if(cut)require(list.pc==list.stall && list.executed_commands==cut && list.state==GeListState::Stalled,"register stream crossed a stall");
                }
                require(callbacks.empty() && list.stack.empty() && list.state==GeListState::Completed,"register batching changed CALL/RET/END");
                const std::array revisions{ge_draw_state_revision,ge_lighting_state_revision,ge_camera_state_revision};
                if(scalar) {reference=ge_state;reference_list=list;reference_revisions=revisions;}
                else {
                    require(ge_state.commands==reference.commands && revisions==reference_revisions,"batched register values/revisions differ from scalar");
                    require(ge_state.transform.projection==reference.transform.projection && ge_state.transform.projection_cursor==reference.transform.projection_cursor,"register stream consumed matrix DATA");
                    require(list.executed_commands==reference_list.executed_commands && list.pc==reference_list.pc && list.histogram==reference_list.histogram,"register batching lost command accounting");
                }
            }
            ge_state=saved;++ge_draw_state_revision;++ge_lighting_state_revision;++ge_camera_state_revision;
        }

        // Exercise the complete list dispatcher, including texture matrices
        // (0x40/0x41), an exact mid-stream stall, reserved cursor wrap, and END.
        {
            const auto saved=ge_state;
            psprecomp::Runtime rt;
            constexpr std::uint32_t pc=0x08050000;
            const std::array<std::uint32_t,9> ops{
                0x4000000eu,ge_float24_command(0x41,91),ge_float24_command(0x41,92),
                ge_float24_command(0x41,0.25f),ge_float24_command(0x41,0.5f),
                0x40000009u,ge_float24_command(0x41,0.75f),
                kGeCommandFinish<<24,kGeCommandEnd<<24};
            for(unsigned i=0;i<ops.size();++i) rt.memory().store32(pc+4*i,ops[i]);
            ge_state=GeState{};reset_ge_transform_state(ge_state.transform);
            GeListRecord list{};list.pc=pc;list.stall=pc+4*4;
            std::vector<GuestCallbackInvocation> callbacks;
            require(execute_ge_list(rt,list,callbacks),"matrix list stall execution failed");
            require(list.pc==list.stall && list.executed_commands==4 && list.state==GeListState::Stalled &&
                ge_state.transform.texture[0]==0.25f && ge_state.transform.texture[1]==0 &&
                ge_state.transform.texture_cursor==1,"matrix stream crossed stall or reserved cursor slots");
            list.stall=0;
            require(execute_ge_list(rt,list,callbacks),"matrix list resume failed");
            require(list.executed_commands==ops.size() && ge_state.transform.texture[1]==0.5f &&
                ge_state.transform.texture[9]==0.75f && ge_state.transform.texture_cursor==10,
                "matrix stream lost data/order on resume");
            ge_state=saved;++ge_draw_state_revision;++ge_lighting_state_revision;++ge_camera_state_revision;
        }
        ...
        // with a timeline resync -- an audible click -- on every submission.
        {
            ...
            const auto fourth_start = first + elapsed_us(6144u);
            virtual_time_us = fourth_start + 55'000u;
            require(audio_queue_buffer(channel, channel.sample_count) == fourth_start,
                    "small scheduler lateness inserted silence into continuous radio");
        ...
        // A capped vblank yields to lower-priority workers until the next
        // frame deadline. Waiting on the host and then delaying by 1 us can
        // reselect the display thread forever (black screen during boot).
        {
            reset();
            psprecomp::Runtime scheduler_runtime;
            GraphicsFramePacer pacer;
            const auto start = GraphicsRealtimeClock::Time{};
            (void)pacer.wait_microseconds(start, 16683u);
            ThreadRecord display{};
            display.name = "display"; display.priority = 16u; display.state = ThreadState::Running;
            thread_table.threads.emplace(1, std::move(display));
            ThreadRecord loader{};
            loader.name = "loader"; loader.priority = 40u; loader.state = ThreadState::Ready;
            thread_table.threads.emplace(2, std::move(loader));
            psprecomp::AllegrexContext loader_context{};
            loader_context.pc = 0x08810000u;
            enqueue_continuation(2, loader_context);
            thread_table.current_uid = 1;
            psprecomp::AllegrexContext display_context{};
            display_context.gpr[31] = 0x08811000u;
            const auto wait = pacer.wait_microseconds(start + std::chrono::microseconds(8000), 16683u);
            require(wait == 8683u, "vblank deadline includes an extra frame interval");
            require(delay_current_thread(scheduler_runtime, display_context, wait) &&
                        thread_table.current_uid == 2 && display_context.pc == loader_context.pc,
                    "vblank wait starved the lower-priority loader");
            virtual_time_us = wait - 1u;
            promote_expired_delays();
            require(thread_table.continuations.empty(), "display thread resumed before vblank");
            virtual_time_us = wait;
            promote_expired_delays();
            require(preempt_if_higher_priority(display_context, "self-test-vblank") &&
                        thread_table.current_uid == 1 && display_context.pc == 0x08811000u,
                    "display thread did not resume at its vblank deadline");
        }
...

        {
            ...
            put16(36u, 34u);
            constexpr std::array<std::uint8_t, 16> atrac_guid{
                0xBF, 0xAA, 0x23, 0xE9, 0x58, 0xCB, 0x71, 0x44,
                0xA1, 0x19, 0xFF, 0xFA, 0x01, 0xE4, 0xCE, 0x62};
            std::copy(atrac_guid.begin(), atrac_guid.end(), atrac_header.begin() + 44u);
