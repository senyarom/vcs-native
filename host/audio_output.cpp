...
#if defined(_WIN32) || defined(PSPRECOMP_USE_SDL)
...
// Do not seal the newest ~70 ms of the guest timeline. Other PSP channels can
...
constexpr std::uint64_t kMixSafetyFrames =
    ((kAudioSchedulingSlackUs * kSampleRate / 1'000'000u + kBlockFrames - 1u) /
     kBlockFrames) * kBlockFrames;
...
struct AudioState {
    ...
    std::uint64_t backlog_frames_discarded{};
...
#else
    constexpr std::size_t bytes_per_block = kBlockFrames * kOutputChannels * sizeof(std::int16_t);
    return (SDL_GetQueuedAudioSize(state.device) + bytes_per_block - 1u) / bytes_per_block;
#endif
}
...
bool ensure_device(AudioState &state) {
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
        std::cerr << "[audio-host] SDL initialization failed: " << SDL_GetError() << "\n";
        state.failed = true;
        return false;
    }
    SDL_AudioSpec format{};
    format.freq = static_cast<int>(kSampleRate);
    format.format = AUDIO_S16SYS;
    format.channels = kOutputChannels;
    format.samples = kBlockFrames;
    state.device = SDL_OpenAudioDevice(nullptr, 0, &format, nullptr, 0);
    if (state.device == 0) {
        std::cerr << "[audio-host] SDL device failed: " << SDL_GetError() << "\n";
        state.failed = true;
        return false;
    }
#endif
    ...
    if (diagnostics_enabled()) {
#if defined(_WIN32)
        const char *driver = "waveOut";
#else
        const char *driver = SDL_GetCurrentAudioDriver();
#endif
        std::cerr << "[audio-host] driver=" << (driver ? driver : "unknown")
                  << " 44100Hz stereo block_frames=" << kBlockFrames
                  << " blocks=" << kBlockCount
...
bool queue_one_block(AudioState &state) {
...
#else
    if (outstanding_blocks(state) >= kBlockCount) return false;
#endif
    ...
    // Diagnose with the real hardware sample clock while keeping the desktop
    // silent. The optional WAV above retains the actual game mix.
    static const bool mute_device = [] {
        const char *value = std::getenv("PSPRECOMP_AUDIO_MUTE_DEVICE");
        return value && *value && std::strcmp(value, "0") != 0;
    }();
    if (mute_device) std::fill(block.samples.begin(), block.samples.end(), 0);
...
#else
    if (SDL_QueueAudio(state.device, block.samples.data(),
                      static_cast<Uint32>(block.samples.size() * sizeof(std::int16_t))) != 0) {
        std::cerr << "[audio-host] SDL queue failed: " << SDL_GetError() << "\n";
        return false;
    }
#endif
...
void advance_locked(AudioState &state, std::uint64_t guest_time_us) {
    ...
    state.guest_now_us = std::max(state.guest_now_us, guest_time_us);
    ...
    const std::uint64_t guest_frame = guest_frame_for(state, state.guest_now_us);
    const auto device_frames = outstanding * kBlockFrames;
    const auto backlog = guest_frame > state.output_frame ? guest_frame - state.output_frame : 0u;
    // A deeper reserve is temporary recovery protection, not the permanent
    // latency budget after playback has restarted.
    const auto latency_blocks = state.recovering_from_underrun
        ? state.recovery_prebuffer_blocks + 2u : state.prebuffer_blocks + 4u;
    const auto max_latency = kMixSafetyFrames + latency_blocks * kBlockFrames;
    if (backlog + device_frames > max_latency) {
        // A stalled host must resume current sound, not play a growing tail of
        // old PCM. Trim only unsubmitted frames, with hysteresis so ordinary
        // callback jitter cannot trigger a cut on every refill.
        const auto target_latency = kMixSafetyFrames + state.prebuffer_blocks * kBlockFrames;
        const auto keep = std::max<std::uint64_t>(kMixSafetyFrames,
            target_latency > device_frames ? target_latency - device_frames : 0u);
        const auto next = guest_frame > keep ? guest_frame - keep : 0u;
        if (next > state.output_frame) {
            const auto discarded = next - state.output_frame;
            for (std::uint64_t i = 0; i < std::min<std::uint64_t>(discarded, kRingFrames); ++i) {
                const auto slot = ((state.output_frame + i) % kRingFrames) * kOutputChannels;
                state.ring[slot] = state.ring[slot + 1] = 0;
            }
            state.backlog_frames_discarded += discarded;
            state.output_frame = next;
        }
    }
    // When only two native blocks remain, waiting the full scheduling margin for every
    ...
    const auto queue_target = state.recovering_from_underrun
        ? state.recovery_prebuffer_blocks : state.prebuffer_blocks;
    while (sealed_frame >= state.output_frame + kBlockFrames &&
           outstanding_blocks(state) < queue_target) {
...
void audio_output_advance(std::uint64_t guest_time_us) {
    ...
    if (summary_diagnostics_enabled() &&
        ...
        const auto latency_frames = outstanding * kBlockFrames +
            (guest_frame > state.output_frame ? guest_frame - state.output_frame : 0u);
        ...
        line << "[audio-summary] guest_us=" << guest_time_us
             ...
             << " latency_ms=" << (latency_frames * 1000u / kSampleRate)
             << " backlog_discarded=" << state.backlog_frames_discarded
...
AudioOutputStatus audio_output_status() {
    auto &state = audio_state();
    std::lock_guard guard(state.mutex);
    if (!state.opened) return {};
    const auto guest_frame = guest_frame_for(state, state.guest_now_us);
    const auto device_frames = outstanding_blocks(state) * kBlockFrames;
    return {guest_frame, state.output_frame, device_frames,
        device_frames + (guest_frame > state.output_frame ? guest_frame - state.output_frame : 0u),
        state.backlog_frames_discarded, state.late_frames_dropped,
        state.timeline_resyncs, state.underrun_rebuffers};
}
...
void audio_output_pause(bool paused) {
    auto &state = audio_state();
    std::lock_guard<std::mutex> guard(state.mutex);
    if (!state.opened || !state.device || !state.playback_started) return;
#if defined(_WIN32)
    if (paused) waveOutPause(state.device); else waveOutRestart(state.device);
#else
    SDL_PauseAudioDevice(state.device, paused ? 1 : 0);
#endif
}
void audio_output_shutdown() {
    AudioState &state = audio_state();
    std::lock_guard<std::mutex> guard(state.mutex);
    if (!state.opened || !state.device) return;
    ...
    state.backlog_frames_discarded = 0u;
    state.timeline_resyncs = 0u;
    state.underrun_rebuffers = 0u;
    state.submit_calls = state.submit_cpu_ns = state.submit_cpu_max_ns = 0u;
}
